// REPLACE THE ENTIRE FILE: frontend/src/components/dashboard/AnomalyDetectionDashboard.js

import React, { useState, useEffect } from 'react';
// This line is the problem - replace this with the correct import
...
const AnomalyDetectionDashboard = ({ scanId }) => {
  ...
  useEffect(() => {
    const fetchAnomalies = async () => {
      ...
      try {
        // Use anomalyService here, not aiService
        ...
        if (response && response.success) {
          ...
          const errorMsg = response && typeof response.error === 'string' ? 
            response.error : 'Failed to fetch anomalies';
          setError(errorMsg);
          setAnomalies([]);
        }
      } catch (error) {
      ...
      {anomalies.length === 0 ? (
        ...
        <div className="anomaly-list">
          {anomalies.map((anomaly, index) => (
            <div key={anomaly.id || `anomaly-${index}`} className={`anomaly-item card mb-3 border-${getSeverityClass(anomaly.severity)}`}>
              <div className={`card-header bg-transparent border-${getSeverityClass(anomaly.severity)}`}>
                <div className="d-flex justify-content-between align-items-center">
                  <h6 className="mb-0">{anomaly.component || 'Unknown Component'}</h6>
              ...
              <div className="card-body">
                <p className="card-text">{anomaly.description || 'No description available'}</p>
                ...
                <div className="mt-2 d-flex justify-content-between">
                  <small className="text-muted">
                    Anomaly Score: {((anomaly.score || 0) * 100).toFixed(1)}%
                  </small>
