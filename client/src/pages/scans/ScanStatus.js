...
const ScanStatus = () => {
  ...
  const fetchScan = useCallback(async () => {
    ...
    try {
      setLoading(true);
      // Use the new combined method to get scan with results
          ...
          <div className="row mt-3">
            <div className="col-md-6">
              <p>
                ...
                <span className={`badge ${getStatusBadgeClass(scanData.status)} ms-2`}>
                  {scanData.status}
                </span>
              </p>
              <p><strong>Scan Types:</strong> {scanData.scan_types.join(', ')}</p>
              <p><strong>Created:</strong> {new Date(scanData.created_at).toLocaleString()}</p>
            </div>
            <div className="col-md-6">
              {scanData.started_at && (
                <p><strong>Started:</strong> {new Date(scanData.started_at).toLocaleString()}</p>
              )}
              {scanData.completed_at && (
                <p><strong>Completed:</strong> {new Date(scanData.completed_at).toLocaleString()}</p>
              )}
