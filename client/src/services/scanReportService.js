...
export const scanReportService = {
  ...
  createReportFromScan: async (scanId) => {
    try {
      ...
      const response = await api.post(`/api/scanner/scans/${scanId}/create-report/`);
  ...
  getReportForScan: async (scanId) => {
    try {
      const response = await api.get(`/api/scanner/scans/${scanId}/report/`);
      return { success: true, data: response.data };
    } catch (error) {
      ...
      return { 
        success: false, 
        error: error.response?.data?.detail || 'Failed to fetch report for scan. Please try again.'
      };
    }
  },
  ...
  syncCompletedScansToReports: async () => {
    try {
      ...
      for (const scan of completedScans) {
        ...
        const reportResponse = await scanReportService.getReportForScan(scan.id);
        
        // If no report exists, create one
        if (!reportResponse.success || !reportResponse.data) {
          const createResponse = await scanReportService.createReportFromScan(scan.id);
          if (createResponse.success) {
            createdReports++;
          } else {
            errors.push(`Failed to create report for scan ${scan.id}: ${createResponse.error}`);
          }
        }
      }
