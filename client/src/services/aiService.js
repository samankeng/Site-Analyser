...
export const aiService = {
  ...
  analyzeScan: async (scanId) => {
    ...
    try {
      const response = await api.post("/ai-analyzer/analyses/analyze/", {
        scan_id: scanId,
      });
      // Remove debug log: console.log('Analysis response:', response);
  ...
  getAnalysesForScan: async (scanId) => {
    ...
    try {
      ...
      const response = await api.get(
        `/ai-analyzer/analyses/for_scan/?scan_id=${scanId}`
      );
      // Remove debug log: console.log('Get analyses response:', response);
      ...
      if (!Array.isArray(analysesData) && typeof analysesData === "object") {
        if (Array.isArray(analysesData.results)) {
          ...
          Object.keys(analysesData).length > 0 &&
          typeof analysesData.id === "string"
        ) {
          ...
          console.warn(
            "Unknown API response format for analyses, defaulting to empty array"
          );
          analysesData = [];
        }
      }
  ...
  getRecommendationsForAnalysis: async (analysisId) => {
    ...
    if (!analysisId || analysisId === "undefined") {
      console.error(
        "Invalid analysisId provided to getRecommendationsForAnalysis:",
        analysisId
      );
    ...
    try {
      // Use proper URL structure matching the backend API
      const response = await api.get(
        `/ai-analyzer/recommendations/for_analysis/?analysis_id=${analysisId}`
      );
      // Remove debug log: console.log('Get recommendations response:', response);
      ...
      if (
        ...
        typeof recommendationsData === "object"
      ) {
        if (Array.isArray(recommendationsData.results)) {
          ...
          Object.keys(recommendationsData).length > 0 &&
          typeof recommendationsData.id === "string"
        ) {
          // If it's a single object with an ID, wrap it in an array
          ...
          // If we can't determine the format, assume empty array
          console.warn(
            "Unknown API response format for recommendations, defaulting to empty array"
          );
          recommendationsData = [];
        }
      }
