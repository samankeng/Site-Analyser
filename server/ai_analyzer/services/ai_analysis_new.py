...
class AIAnalysisService:
    """Service for performing AI-based security analysis with LLM integration"""
    ...
    def __init__(self, scan):
        ...
        self.llm_service = LLMService()  # New LLM service
        ...
        try:
            ...
            analysis = AIAnalysis.objects.create(
                ...
                analysis_result={
                    ...
                    'anomaly_detection': {},
                    'risk_scoring': {},
                    'llm_analysis': {},  # New LLM analysis section
            ...
            # Run LLM analysis - NEW
            logger.info("Starting LLM security analysis")
            try:
                llm_results = self._run_llm_analysis(scan_results, analysis)
                logger.info("LLM analysis completed successfully")
                
                # Update analysis with LLM results
                analysis.analysis_result['llm_analysis'] = llm_results
                analysis.save()
            except Exception as e:
                logger.error(f"Error in LLM analysis: {str(e)}")
                logger.error(traceback.format_exc())
            
            # Generate executive summary with LLM - NEW
            logger.info("Generating executive summary with LLM")
            try:
                executive_summary = self._generate_executive_summary(analysis)
                
                # Add executive summary to analysis result
                analysis.analysis_result['executive_summary'] = executive_summary
                analysis.save()
            ...
            try:
                ...
                confidence_scores = [
                    ...
                    analysis.analysis_result.get('llm_analysis', {}).get('confidence', 0.9)  # Include LLM confidence
                ]
    ...
    def _run_llm_analysis(self, scan_results, analysis):
        """Run LLM-based security analysis - NEW"""
        try:
            logger.info("Preparing data for LLM security analysis")
            
            # Prepare scan data for LLM analysis
            scan_data = {
                'target_url': self.scan.target_url,
                'results': []
            }
            
            # Add headers data if available
            headers_results = scan_results.filter(category='headers').first()
            if headers_results and headers_results.details:
                scan_data['headers'] = headers_results.details
            
            # Add SSL info if available
            ssl_results = scan_results.filter(category='ssl').first()
            if ssl_results and ssl_results.details:
                scan_data['ssl_info'] = ssl_results.details
            ...
            for result in scan_results:
                try:
                    ...
                    logger.warning(f"Error processing result {result.id} for LLM analysis: {str(e)}")
            
            # Perform LLM analysis
            logger.info("Calling LLM service for vulnerability analysis")
            llm_analysis = self.llm_service.analyze_vulnerabilities(scan_data)
            
            # Log LLM analysis results
            vulnerabilities = llm_analysis.get('vulnerabilities', [])
            logger.info(f"LLM found {len(vulnerabilities)} vulnerabilities")
            
            # Generate recommendations based on LLM analysis
            if vulnerabilities:
                logger.info("Generating LLM-based recommendations")
                llm_recommendations = self.llm_service.generate_recommendations(vulnerabilities)
                ...
                for rec in llm_recommendations:
                    try:
                        AIRecommendation.objects.create(
                            ...
                            recommendation_type='llm',
                            confidence_score=rec.get('confidence_score', 0.85)
                        )
                        recs_created += 1
                    except Exception as e:
                        logger.error(f"Error creating LLM recommendation: {str(e)}")
                
                logger.info(f"Created {recs_created} LLM-based recommendations")
                
                # Add recommendations to LLM analysis results
                llm_analysis['recommendations'] = llm_recommendations
    ...
    def _generate_executive_summary(self, analysis):
        """Generate an executive summary using LLM - NEW"""
        try:
            logger.info("Preparing data for executive summary generation")
            
            # Prepare scan data for summary generation
            summary_data = {
                'target_url': self.scan.target_url,
                'risk_level': analysis.analysis_result.get('llm_analysis', {}).get('risk_level', 
                              analysis.analysis_result.get('risk_scoring', {}).get('overall_score', 0)),
                'vulnerabilities': analysis.analysis_result.get('llm_analysis', {}).get('vulnerabilities', []),
                'recommendations': analysis.analysis_result.get('llm_analysis', {}).get('recommendations', [])
            }
            
            # Call LLM service to generate executive summary
            logger.info("Calling LLM service for executive summary generation")
            summary = self.llm_service.generate_executive_summary(summary_data)
            
            return summary
                
        except Exception as e:
            logger.exception(f"Error generating executive summary: {str(e)}")
            return "Error generating executive summary. Please check the detailed analysis results."
